...
import { Package, ArrowDown, ArrowUp, RefreshCw, Search, Plus, ChefHat, ClipboardCheck, Download } from "lucide-react";
...
import { exportKitchenStorePDF, exportPhysicalCountPDF } from "@/lib/pdfExport";
import { toast } from "sonner";

export default function KitchenStorePage() {
  const { kitchenStock, refresh, addQty, deductQty, totalValue } = useKitchenStore();
  ...
  const handleAdjust = (ingredientId: string) => {
    ...
    if (adjustType === "add") {
      ...
      toast.success(`Added ${qty} to kitchen stock`);
    } else {
      deductQty(ingredientId, qty);
      toast.success(`Deducted ${qty} from kitchen stock`);
    }
    setShowAdjust(null);
    setAdjustQty("");
    setAdjustNote("");
  ...
  const handleSavePhysicalCount = () => {
    const today = new Date().toISOString().split("T")[0];
    const entries: PhysicalCountEntry[] = kitchenStock.map(k => {
    ...
    toast.success(`Kitchen physical count saved — ${shortageCount} shortage(s), ${overageCount} overage(s)`);
  };

  const handleExportPDF = () => {
    const stockData = kitchenStock.map(k => ({
      ingredientName: k.ingredientName,
      unit: k.unit,
      costPerUnit: k.costPerUnit,
    ...
    exportKitchenStorePDF(stockData, settings.hotelName, user?.name ?? "System", pcData);
    toast.success("Kitchen store PDF exported");
  };
  ...
  const emptyItems = kitchenStock.filter(k => k.currentQuantity === 0).length;
  const lowItems = kitchenStock.filter(k => k.currentQuantity > 0 && k.currentQuantity < 1).length;
  const today = new Date().toISOString().split("T")[0];
    ...
    <AppLayout>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <div className="stat-card text-center">
          <p className="text-xl font-bold font-mono text-foreground">{kitchenStock.length}</p>
          <p className="text-xs text-muted-foreground mt-0.5">Items in Kitchen</p>
        </div>
        ...
        <div className="stat-card text-center">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Kitchen Stock Value</p>
        </div>
      </div>
      ...
      <div className="mb-4 flex items-start gap-3 rounded-xl border border-primary/20 bg-primary/5 px-4 py-3">
        ...
        <p className="text-xs text-primary/80">
          Kitchen stock is automatically updated: <strong>+</strong> when items are transferred/issued to Kitchen/Bar,
          <strong> −</strong> when sales are recorded (recipe ingredients) and manual consumption is logged.
        </p>
      </div>

      {/* Today's physical count banner */}
      {savedCount && (
        <div className="mb-4 flex items-center gap-3 rounded-xl border border-green-500/20 bg-green-500/5 px-4 py-3">
          <ClipboardCheck className="w-4 h-4 text-green-400 shrink-0" />
          <div className="flex-1">
            <p className="text-xs font-semibold text-green-300">Kitchen physical count completed today by {savedCount.countedByName}</p>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={handleExportCountPDF} className="text-[10px] text-green-400 border border-green-500/30 px-2 py-1 rounded-lg hover:bg-green-500/10 flex items-center gap-1">
            ...
            {canCount && (
              ...
              }} className="text-[10px] text-green-400 underline">Re-count</button>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            placeholder="Search kitchen stock..."
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {canCount && (
            <button onClick={() => { setPhysicalCounts({}); setCountNotes({}); setShowPhysicalCount(true); }}
              className="flex items-center gap-2 px-4 py-1.5 rounded-lg bg-green-600 text-white text-xs font-semibold hover:bg-green-700">
      ...
      {kitchenStock.length === 0 ? (
        <div className="text-center py-20 text-muted-foreground">
          ...
          <p className="text-base font-semibold text-foreground">Kitchen Store is Empty</p>
          <p className="text-sm mt-2 max-w-sm mx-auto">
            Transfer ingredients from the Main Store to Kitchen using Stock Movements → Issue/Transfer to Kitchen.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                {["Ingredient", "Unit", "Cost/Unit", "Kitchen Stock",
                ...
                return (
                  <tr key={item.ingredientId} className={cn(
                    ...
                    <td className="px-4 py-3 text-xs font-mono text-foreground">
                      {formatCurrency(item.currentQuantity * item.costPerUnit, "ETB")}
                    </td>
                    <td className="px-4 py-3">
                      {isOut ? (
                    ...
                    {canEdit && (
                      <td className="px-4 py-3">
                        <button
                          ...
                          className="flex items-center gap-1 text-[10px] text-primary hover:bg-primary/10 px-2 py-1 rounded transition-colors"
                        >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-2xl w-full max-w-4xl shadow-2xl fade-in max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-5 border-b border-border">
              <div>
                <h2 className="font-semibold text-foreground flex items-center gap-2">
                  <ChefHat className="w-4 h-4 text-green-400" /> Kitchen Physical Inventory Count
                </h2>
                <p className="text-xs text-muted-foreground mt-0.5">Count each ingredient physically. Variance = Physical − Theoretical.</p>
              </div>
                  ...
                  {kitchenStock.map(k => {
                    ...
                    return (
                      <tr key={k.ingredientId} className={cn("border-b border-border/50 last:border-0",
                        ...
                        <td className="px-4 py-2.5 text-xs font-bold font-mono text-blue-400">{k.currentQuantity.toFixed(3)} {k.unit}</td>
                        <td className="px-4 py-2.5">
                          <input
                            ...
                            className="w-24 px-2 py-1 text-xs font-mono rounded-lg bg-input border border-border text-foreground focus:outline-none focus:ring-1 focus:ring-green-500"
                          />
                        </td>
                        <td className="px-4 py-2.5">
                          {variance !== null ? (
            ...
            <div className="flex gap-3 p-5 border-t border-border">
              ...
              <button onClick={handleSavePhysicalCount} className="flex-1 py-2 rounded-lg bg-green-600 text-white text-sm font-semibold hover:bg-green-700">
                <ClipboardCheck className="w-4 h-4 inline mr-2" />Save Kitchen Count
              </button>
            </div>
          </div>
        </div>
      ...
      {showAdjust && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-2xl w-full max-w-sm shadow-2xl fade-in">
            <div className="flex items-center justify-between p-5 border-b border-border">
              <h2 className="font-semibold text-foreground">Adjust Kitchen Stock</h2>
              <button onClick={() => setShowAdjust(null)} className="text-muted-foreground hover:text-foreground text-lg">×</button>
            </div>
            <div className="p-5 space-y-4">
              <p className="text-xs font-semibold text-foreground">
                {kitchenStock.find(k => k.ingredientId === showAdjust)?.ingredientName}
              </p>
              <div className="flex gap-2">
                <button onClick={() => setAdjustType("add")}
                  className={cn("flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg border text-xs font-semibold transition-all",
                    adjustType === "add" ? "bg-green-500/20 border-green-500/40 text-green-400" : "bg-muted/30 border-border text-muted-foreground")}>
                  <ArrowDown className="w-3.5 h-3.5" /> Add Stock
                </button>
                <button onClick={() => setAdjustType("deduct")}
                  className={cn("flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg border text-xs font-semibold transition-all",
                    adjustType === "deduct" ? "bg-red-500/20 border-red-500/40 text-red-400" : "bg-muted/30 border-border text-muted-foreground")}>
                  <ArrowUp className="w-3.5 h-3.5" /> Deduct
                </button>
              </div>
              <div>
              ...
              <div>
                ...
                <input value={adjustNote} onChange={e => setAdjustNote(e.target.value)}
                  className="w-full px-3 py-2 text-sm rounded-lg bg-input border border-border text-foreground focus:outline-none focus:ring-1 focus:ring-primary" placeholder="Optional note" />
              </div>
            </div>
            <div className="flex gap-3 p-5 border-t border-border">
